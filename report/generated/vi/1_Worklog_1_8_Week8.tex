\vspace{0.5em}\noindent\textbf{Tuần 8 - Triển khai end-to-end và kiểm tra vận
hành}\par

\vspace{0.5em}\noindent\textbf{Mục tiêu}\par

Trong tuần 8, tôi hoàn thiện phần đóng góp Cloud/DevOps của mình trong
project nhóm 5 thành viên. Tôi tích hợp và kiểm tra mô hình production
cho các service của nhóm trên Amazon EKS, Application Load Balancer,
S3/CloudFront, các dịch vụ dữ liệu AWS managed, event-driven processing
và cost tracking.

Tôi sử dụng thư mục minh chứng AWS tuần 08 làm nguồn dữ liệu cho phần
này. Số liệu chi phí lấy từ tổng hợp ngày 01-28/07/2026, screenshot
Billing credits và Cost Explorer. Hóa đơn tháng đã chốt và AWS Pricing
Calculator vẫn chưa có.
...
\bottomrule\noalign{}
\endlastfoot
Hoàn thành & Tích hợp AI service theo hướng SageMaker do thành viên AI
cung cấp vào deployment path. & Source implementation và
\texttt{sagemaker-\allowbreak{}health.\allowbreak{}png}. \\
Hoàn thành & Sửa image propagation của \texttt{deploy-\allowbreak{}app} và cách chọn
ECR image xác định. & CI/CD workflow implementation. \\
Hoàn thành & Loại frontend khỏi Kubernetes runtime để phân phối qua S3
và CloudFront. & Deployment script và architecture note. \\
Hoàn thành & Kiểm tra trạng thái Amazon RDS PostgreSQL và ElastiCache
Valkey từ screenshot AWS CLI. & \texttt{RDS-\allowbreak{}health.\allowbreak{}png},
\texttt{REDIS-\allowbreak{}health.\allowbreak{}png}. \\
Hoàn thành & Kiểm tra trạng thái SageMaker endpoint. &
\texttt{sagemaker-\allowbreak{}health.\allowbreak{}png}. \\
Hoàn thành một phần & Kiểm tra EKS workload và ALB runtime. & EKS
workload chạy nhưng screenshot ALB còn target không healthy. \\
Hoàn thành & Bổ sung minh chứng runtime CloudFront, S3, DynamoDB và SQS.
& Các screenshot đã được xử lý thông tin nhạy cảm. \\
Hoàn thành một phần & Bổ sung minh chứng chi phí AWS ngày 01-28/07/2026.
& Cost summary và screenshot; Pricing Calculator còn bị chặn. \\
\end{longtable}
\endgroup
}
...
\textbf{Mô tả sơ bộ.} Điểm vào production là CloudFront: yêu cầu
frontend tĩnh đi đến S3, còn API và chat đi qua ALB đến EKS. Dữ liệu ứng
dụng được lưu tại RDS, Redis và DynamoDB; xử lý CV bất đồng bộ dùng
worker, AI service và SageMaker; sự kiện outbox đi đến SQS và Lambda
thông báo.
...
\vspace{0.5em}\noindent\textbf{Minh chứng vận hành
AWS}\par
...
\vspace{0.5em}\noindent\textbf{Tổng hợp minh chứng vận
hành}\par
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Amazon EKS runtime pods and services}]{../static/logs/worklog/week-08/aws/eks-runtime-pods-2026-07-28.png}}
\caption{Amazon EKS runtime pods and services}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Amazon EKS deployment rollout success}]{../static/logs/worklog/week-08/aws/eks-deployment-rollout-success.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Application Load Balancer target health}]{../static/logs/worklog/week-08/aws/ALB-target-health.png}}
\caption{Application Load Balancer target health}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Application Load Balancer target groups}]{../static/logs/worklog/week-08/aws/ALB-target.png}}
\caption{Application Load Balancer target groups}
\end{figure}

\vspace{0.5em}\noindent\textbf{Amazon CloudFront và
S3}\par
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={CloudFront monitoring evidence}]{../static/logs/worklog/week-08/aws/Cloudfront-evidence.png}}
\caption{CloudFront monitoring evidence}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={S3 frontend bucket object evidence}]{../static/logs/worklog/week-08/aws/S3-evidenc.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Amazon RDS PostgreSQL health}]{../static/logs/worklog/week-08/aws/RDS-health.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Amazon ElastiCache Valkey health}]{../static/logs/worklog/week-08/aws/REDIS-health.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={DynamoDB table status evidence}]{../static/logs/worklog/week-08/aws/DynamoDB-evidenc.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={SQS queue evidence}]{../static/logs/worklog/week-08/aws/SQS-evidence.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWS Lambda function configuration}]{../static/logs/worklog/week-08/aws/lambda.png}}
\caption{AWS Lambda function configuration}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWS Lambda source evidence}]{../static/logs/worklog/week-08/aws/Lambda-evidence.png}}
\caption{AWS Lambda source evidence}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWS Lambda CloudWatch log group evidence}]{../static/logs/worklog/week-08/aws/lambda-cloudwatch-evidence.png}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Amazon SageMaker endpoint status}]{../static/logs/worklog/week-08/aws/sagemaker-health.png}}
\caption{Amazon SageMaker endpoint status}
\end{figure}

\vspace{0.5em}\noindent\textbf{Minh chứng chi phí
AWS}\par
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWS Billing credits summary for July 2026}]{../static/logs/worklog/week-08/aws/total-cost-july.png}}
\caption{AWS Billing credits summary for July 2026}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={AWS Cost Explorer overview evidence}]{../static/logs/worklog/week-08/aws/Cost-evidence.png}}
\caption{AWS Cost Explorer overview evidence}
\end{figure}

\vspace{0.5em}\noindent\textbf{Tổng hợp chi phí từ ngày 1 đến 28 tháng
7}\par
...
\vspace{0.5em}\noindent\textbf{Xu hướng chi phí hằng
ngày}\par
...
\vspace{0.5em}\noindent\textbf{Các dịch vụ phát sinh chi phí cao
nhất}\par
...
\vspace{0.5em}\noindent\textbf{Các bất thường về chi
phí}\par
...
\vspace{0.5em}\noindent\textbf{Các nhóm đích ALB chưa hoàn toàn khỏe
mạnh}\par
...
\vspace{0.5em}\noindent\textbf{Minh chứng chi phí AWS đã có nhưng chưa đầy
đủ}\par
...
\vspace{0.5em}\noindent\textbf{Đã bổ sung minh chứng vận hành cho một số dịch vụ
AWS}\par
...
\vspace{0.5em}\noindent\textbf{Ảnh chụp Lambda cần được xử lý thông tin nhạy
cảm}\par
...
Phần đóng góp cá nhân tuần 8 tạo được bộ minh chứng AWS runtime cho
compute và data path chính:
...
Ứng dụng end-to-end vẫn là kết quả chung của nhóm. Trong phạm vi của
tôi, các khoảng trống còn lại được ghi rõ: ALB target chưa hoàn toàn
healthy, còn thiếu CloudFront behavior/origin export và browser smoke
test, đồng thời số liệu chi phí cần hóa đơn đã chốt và Pricing
Calculator.

\vspace{0.5em}\noindent\textbf{Công việc còn lại}\par
